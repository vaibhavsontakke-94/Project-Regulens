...
export function createSihStore(options = {}) {
  ...
  return {
    ...
    createProblem, listProblems, getProblem, patchProblem, countProblems,
...
async function getPilotIntelligence(id, pilotId) {
  return adapter.get("pilot_intelligence", { pilotId, id });
}
async function listPilotIntelligence(pilotId, opts = {}) {
  return adapter.select("pilot_intelligence", { pilotId }, { order: opts.order || "createdAt", orderDir: opts.orderDir || "desc", limit: opts.limit || 50 });
}
async function patchPilotIntelligence(id, pilotId, patch) {
  return adapter.patch("pilot_intelligence", { pilotId, id }, { ...patch, updatedAt: now() });
}