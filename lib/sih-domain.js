...
export const EVALUATION_DECISION_TYPES = [
  ...
  "ACCEPT_RECOMMENDATION", "ACCEPT_WITH_CONDITIONS", "REQUEST_FURTHER_VALIDATION", "REJECT",
];
export const EVALUATION_DECISION_STAGES = ["EVALUATION", "PILOT_SELECTION", "REVIEW", "PROCUREMENT"];
/* Procurement-stage decisions are recorded only after a completed, analysed pilot
   against the assessed readiness state (READY / READY_WITH_CONDITIONS / etc.). */
export const PROCUREMENT_DECISION_TYPES = [
  "ACCEPT_RECOMMENDATION", "ACCEPT_WITH_CONDITIONS", "REQUEST_FURTHER_VALIDATION", "REJECT",
];
...
export function computePilotPerformanceSummary(pilot) {
  const overallScore = Math.round(Number(pilot && pilot.overallScore) || 0);
  const kpiCount = Number(pilot && pilot.kpiCount) || 0;
  const milestoneCount = Number(pilot && pilot.milestoneCount) || 0;
  const targetAchievement = Math.round(Number(pilot && pilot.targetAchievement) || 0);
  const health = computePilotHealth(overallScore, kpiCount, milestoneCount, targetAchievement);
  const alerts = [];
  if (overallScore < 30) alerts.push("overall score is critically low");
  else if (overallScore < 50) alerts.push("overall score is below the healthy band");
  if (kpiCount > 0 && targetAchievement < 40) alerts.push("target achievement is critically low");
  else if (targetAchievement < 60) alerts.push("target achievement is below the healthy band");
  if (!alerts.length) alerts.push("no material performance concerns");
  return { health, overallScore, targetAchievement, kpiCount, milestoneCount, alerts };
}
