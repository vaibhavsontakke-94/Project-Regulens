...
export function generateProcurementReadinessSummary({problem, startup, pilot, pilotResult, compliance, risks}) {
  ...
  */
  ...
  const riskSummary = risks.risks ? {
    ...
    top risks: (risks.risks || []).slice(0, 3).map(r => ({ title: r.title, severity: r.severity, category: r.category }))
  } : { totalRisks: 0, criticalHighCount: 0, topRisks: [] };
...
export function computePilotPerformanceSummary(pilot) {
  const overallScore = pilot.overallScore != null ? Number(overallScore) : 0;
  const kpiCount = pilot.kpiCount != null ? Number(pilot.kpiCount) : 0;
  const milestoneCount = pilot.milestoneCount != null ? Number(pilot.milestoneCount) : 0;
  const targetAchievement = pilot.targetAchievement != null ? Number(pilot.targetAchievement) : 0;

  const health = computePilotHealth(overallScore, kpiCount, milestoneCount, targetAchievement);

  const alerts = [];
  if (targetAchievement < 70) alerts.push("KPI below target");
  if (overallScore < 50) alerts.push("Declining performance");
  if (milestoneCount > 0 && /* check for delayed milestones */ true) alerts.push("Milestone progress review");
  if (pilot.budget && pilot.spentBudget && pilot.spentBudget > pilot.budget * 0.8) alerts.push("Budget anomaly");
  if (kpiCount === 0) alerts.push("Missing data: no KPIs defined");
  ...
  const summary = {
    ...
    milestoneCount,
    alerts,
    /* Achievement % per KPI would be computed from actual KPI data */
  };
