...
export const PROBLEM_TRANSITIONS = {
  ...
  SUBMITTED: ["APPROVED", "REVIEW", "ARCHIVED"],
  APPROVED: ["IN_CHALLENGE", "ARCHIVED"],
  IN_CHALLENGE: ["ARCHIVED"],
  ARCHIVED: [],
};
...
const QUALITY_RULES = [
  ...
  { key: "geography", label: "geography", has: (p) => !!String(p.geography || "").trim(), blocking: false },
...
export function deterministicStructure(problem) {
  ...
  out.current_state = String(p.currentState || "").trim();
  out.desired_state = String(p.desiredState || "").trim();
...
export function withProvenance(structure, problem) {
  ...
  const prov = (text) => {
    ...
    const userText = [
      ...
      p.constraints, p.dataAvailability, Array.isArray(p.technologyPreferences) ? p.technologyPreferences.join(" ") : "",
    ].join(" ").toLowerCase();
